...
CREATE OR REPLACE FUNCTION zdb.highlight_document(index regclass, document jsonb, query_string text, dedup_results bool DEFAULT '(('t')::pg_catalog.bool)') RETURNS TABLE(field_name text, array_index pg_catalog.int4, term text, type text, "position" pg_catalog.int4, start_offset pg_catalog.int8, end_offset pg_catalog.int8, query_clause text) AS 'MODULE_PATHNAME', 'highlight_document_jsonb_wrapper' IMMUTABLE LANGUAGE c PARALLEL SAFE STRICT;
DROP FUNCTION IF EXISTS zdb.highlight_document(index regclass, document json, query_string text);
CREATE OR REPLACE FUNCTION zdb.highlight_document(index regclass, document json, query_string text, dedup_results bool DEFAULT '(('t')::pg_catalog.bool)') RETURNS TABLE(field_name text, array_index pg_catalog.int4, term text, type text, "position" pg_catalog.int4, start_offset pg_catalog.int8, end_offset pg_catalog.int8, query_clause text) AS 'MODULE_PATHNAME', 'highlight_document_json_wrapper' IMMUTABLE LANGUAGE c PARALLEL SAFE STRICT;
