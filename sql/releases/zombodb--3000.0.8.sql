...
CREATE OR REPLACE FUNCTION zdb."restrict"(
	"root" internal, /* pgrx::datum::internal::Internal */
...
CREATE OR REPLACE FUNCTION zdb."score"(
	"ctid" tid /* core::option::Option<pgrx_pg_sys::pg13::ItemPointerData> */
) RETURNS double precision /* f64 */
IMMUTABLE PARALLEL SAFE
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."point_to_json"(
	"point" point /* pgrx_pg_sys::pg13::Point */
) RETURNS json /* pgrx::datum::json::Json */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."point_array_to_json"(
	"points" point[] /* pgrx::datum::array::Array<pgrx_pg_sys::pg13::Point> */
) RETURNS json /* pgrx::datum::json::Json */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."ctid"(
	"as_u64" bigint /* i64 */
) RETURNS tid /* pgrx_pg_sys::pg13::ItemPointerData */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
--
-- Overridden as (due to a `///` comment with a `pgrxsql` code block):
...
-- CREATE OR REPLACE FUNCTION zdb."amhandler"() RETURNS internal /* pgrx::pgbox::PgBox<pgrx_pg_sys::pg13::IndexAmRoutine> */
-- STRICT
-- LANGUAGE c /* Rust */
...
) RETURNS numeric /* pgrx::datum::numeric::Numeric */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."percentiles"(
	...
	"value" numeric  /* pgrx::datum::numeric::Numeric */
)
IMMUTABLE PARALLEL SAFE
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."stats"(
	...
	"count" bigint,  /* i64 */
	"min" numeric,  /* pgrx::datum::numeric::Numeric */
	"max" numeric,  /* pgrx::datum::numeric::Numeric */
	"avg" numeric,  /* pgrx::datum::numeric::Numeric */
	"sum" numeric  /* pgrx::datum::numeric::Numeric */
)
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."range"(
	...
	"from" numeric,  /* core::option::Option<pgrx::datum::numeric::Numeric> */
	"to" numeric,  /* core::option::Option<pgrx::datum::numeric::Numeric> */
...
CREATE OR REPLACE FUNCTION zdb."query_tids"(
	"index" regclass, /* pgrx::rel::PgRelation */
	"query" pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
) RETURNS tid[] /* alloc::vec::Vec<pgrx_pg_sys::pg13::ItemPointerData> */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."query"(
	"index" regclass, /* pgrx::rel::PgRelation */
	"query" pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
) RETURNS SETOF tid /* pgrx_pg_sys::pg13::ItemPointerData */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."top_hits"(
	...
	"id" tid,  /* pgrx_pg_sys::pg13::ItemPointerData */
	"score" double precision,  /* f64 */
	"source" json  /* pgrx::datum::json::Json */
)
IMMUTABLE PARALLEL SAFE STRICT
...
CREATE OR REPLACE FUNCTION zdb."matrix_stats"(
	...
	"term" text,  /* alloc::string::String */
	"count" bigint,  /* i64 */
	"mean" numeric,  /* pgrx::datum::numeric::Numeric */
	"variance" numeric,  /* pgrx::datum::numeric::Numeric */
	"skewness" numeric,  /* pgrx::datum::numeric::Numeric */
	"kurtosis" numeric,  /* pgrx::datum::numeric::Numeric */
...
CREATE OR REPLACE FUNCTION zdb."histogram"(
	...
	"term" numeric,  /* pgrx::datum::numeric::Numeric */
	"doc_count" bigint  /* i64 */
)
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."extended_stats"(
	...
	"avg" numeric,  /* pgrx::datum::numeric::Numeric */
	"sum" numeric,  /* pgrx::datum::numeric::Numeric */
	"sum_of_squares" numeric,  /* pgrx::datum::numeric::Numeric */
	"variance" numeric,  /* pgrx::datum::numeric::Numeric */
	"std_deviation" numeric,  /* pgrx::datum::numeric::Numeric */
	"upper" numeric,  /* pgrx::datum::numeric::Numeric */
	"lower" numeric  /* pgrx::datum::numeric::Numeric */
)
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC smallint[] /* pgrx::datum::array::Array<i16> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC double precision[] /* pgrx::datum::array::Array<f64> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC bigint[] /* pgrx::datum::array::Array<i64> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC integer[] /* pgrx::datum::array::Array<i32> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC bool[] /* pgrx::datum::array::Array<bool> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC real[] /* pgrx::datum::array::Array<f32> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."terms"(
	"field" text, /* &str */
	"values" VARIADIC text[] /* pgrx::datum::array::Array<&str> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
 IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."span_or"(
	"clauses" VARIADIC pg_catalog.ZDBQuery[] /* pgrx::datum::array::Array<zombodb::zdbquery::pg_catalog::ZDBQuery> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."sort_many"(
	"zdbquery" pg_catalog.ZDBQuery, /* zombodb::zdbquery::pg_catalog::ZDBQuery */
	"sort_descriptors" VARIADIC pg_catalog.SortDescriptor[] /* pgrx::datum::array::Array<zombodb::zdbquery::pg_catalog::SortDescriptor> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."geo_polygon"(
	"field" text, /* &str */
	"points" VARIADIC point[] /* pgrx::datum::array::Array<pgrx_pg_sys::pg13::Point> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."geo_bounding_box"(
	"field" text, /* &str */
	"bounding_box" box, /* pgrx_pg_sys::pg13::BOX */
...
CREATE OR REPLACE FUNCTION dsl."bool"(
	"parts" VARIADIC pg_catalog.BoolQueryPart[] /* pgrx::datum::array::Array<zombodb::query_dsl::bool::pg_catalog::BoolQueryPart> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION dsl."not"(
	"queries" VARIADIC pg_catalog.ZDBQuery[] /* pgrx::datum::array::Array<zombodb::zdbquery::pg_catalog::ZDBQuery> */
) RETURNS pg_catalog.ZDBQuery /* zombodb::zdbquery::pg_catalog::ZDBQuery */
IMMUTABLE PARALLEL SAFE STRICT
LANGUAGE c /* Rust */
...
CREATE OR REPLACE FUNCTION zdb."highlight"(
	"ctid" tid, /* pgrx_pg_sys::pg13::ItemPointerData */
	"field" text, /* &str */
