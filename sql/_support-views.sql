...
SELECT (select array_to_string(array_agg(alias), ',') from zdb.cat_aliases where index = index_name) as alias,
       index_name,
       url,
       table_name,
       ...
       (zdb.request(indexrelid, '_count', 'GET', NULL, true) :: JSON) -> 'count'                     AS doc_count,
       coalesce(json_array_length((zdb.request(indexrelid, '_doc/zdb_aborted_xids', 'GET', NULL, true) :: JSON) ->
