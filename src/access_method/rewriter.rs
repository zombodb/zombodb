        ...
        if !context.replacements.is_empty() {
            for telist in context.targetlists {
                ...
                for te in telist.iter_ptr() {
                    ...
                    let expr = te.expr;
                    if is_a(expr as NodePtr, pg_sys::NodeTag::T_Var) {
                                ...
                                if lang == pg_sys::get_language_oid(C.as_ptr(), false) {
                                    ...
                                    if src.trim() == "shadow_wrapper" {
                                        ...
                                        if is_a(first_arg, pg_sys::NodeTag::T_Var)
                                            && pg_sys::get_func_rettype(funcexpr.funcid)
...
unsafe fn walk_plan(plan: *mut pg_sys::Plan, context: &mut WalkContext) {
    ...
    let is_modify = is_a(plan.as_ptr() as NodePtr, pg_sys::NodeTag::T_ModifyTable);
    if is_modify {
...
unsafe fn walk_node(node: NodePtr, context: &mut WalkContext) {
    ...
    if is_a(node, pg_sys::NodeTag::T_PlannedStmt) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_SeqScan) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_IndexScan) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_IndexOnlyScan) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_Aggref) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_BitmapAnd) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_BitmapOr) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_MergeAppend) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_MergeJoin) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_WorkTableScan) {
        let mut scan = PgBox::from_pg(node as *mut pg_sys::WorkTableScan);
        walk_plan(&mut scan.scan.plan, context);
    } else if is_a(node, pg_sys::NodeTag::T_Result) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_Sort) {
    ...
    } else if is_a(node, pg_sys::NodeTag::T_CaseWhen) {
        ...
        #[cfg(any(feature = "pg14", feature = "pg15"))]
        if is_a(node, pg_sys::NodeTag::T_Memoize) {
