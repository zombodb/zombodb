...
static bool lookup_type_mapping(Oid typeOid, char *fieldname, StringInfo mapping) {
	...
	if (SPI_processed > 1) {
		...
		appendStringInfo(mapping, ", \"%s\":%s", fieldname, json);
		rc = true;
	}
...
StringInfo generate_mapping(Relation heapRel, TupleDesc tupdesc) {
	...
	for (i = 0; i < tupdesc->natts; i++) {
		...
		if (lookup_field_mapping(RelationGetRelid(heapRel), NameStr(attr->attname), mapping))
			continue;

		/* apply type-specific mapping, if we have it in the database */
		if (lookup_type_mapping(typeOid, NameStr(attr->attname), mapping))
			continue;
