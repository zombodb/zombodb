...
pub fn extern_block(block: ItemForeignMod) -> proc_macro2::TokenStream {
    ...
    for item in block.items.into_iter() {
        stream.extend(foreign_item(item));
    }

    stream
}

pub fn foreign_item(item: ForeignItem) -> proc_macro2::TokenStream {
    match item {
        ForeignItem::Fn(func) => {
            ...
            foreign_item_fn(func)
        }
        _ => quote! { extern "C" { #item } },
    }
}

pub fn foreign_item_fn(func: ForeignItemFn) -> proc_macro2::TokenStream {
