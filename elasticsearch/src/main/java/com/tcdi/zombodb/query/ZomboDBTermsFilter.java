 ...
 *
 * Copied from @{link org.apache.lucene.queries.TermsFilter} and modified to take a Set<BytesRef> in the constructor
 */
final class ZomboDBTermsFilter extends Filter {
    ...
    /**
     * Creates a new {@link ZomboDBTermsFilter} from the given {@link BytesRef} list for
     * a single field.
     */
    ZomboDBTermsFilter(final String field, final Set<BytesRef> terms) {
        this(new FieldAndTermEnum(field) {
            // we need to sort for deduplication and to have a common cache key
            final Iterator<BytesRef> iter = terms.iterator();
            @Override
    ...
    private ZomboDBTermsFilter(FieldAndTermEnum iter, int length) {
    ...
    public boolean equals(Object obj) {
        ...
        ZomboDBTermsFilter test = (ZomboDBTermsFilter) obj;
