...
if [ "${PKG_FORMAT}" == "deb" ]; then
	...
	fpm \
		...
		-a $(uname -m) \
		./home/$(whoami)/postgresql/${PGVER}/build/share/extension=/usr/share/postgresql/ \
		./home/$(whoami)/postgresql/${PGVER}/build/lib/zombodb.so=/usr/lib/postgresql/ \
		|| exit 1
	...
	fpm \
		...
		-n zombodb-${PGVER} \
		-v ${VERSION} \
		-p ${BUILDDIR}/target/release/zombodb_${OSNAME}_postgres_${PGVER}-${VERSION}.$(uname -m).apk \
		-a $(uname -m) \
		./home/$(whoami)/postgresql/${PGVER}/build/share/extension=/usr/local/share/postgresql/ \
		./home/$(whoami)/postgresql/${PGVER}/build/lib/zombodb.so=/usr/local/lib/postgresql/ \
		|| exit 1
