...
PG_FUNCTION_INFO_V1(zdb_internal_dump_query_tree);
...
Datum zdb_internal_dump_query_tree(PG_FUNCTION_ARGS) {
    Oid                indexRel   = PG_GETARG_OID(0);
    char               *userQuery = GET_STR(PG_GETARG_TEXT_P(1));
    ZDBIndexDescriptor *desc;
    char               *jsonQuery;
    ...
    jsonQuery = desc->implementation->dumpQueryTree(desc, userQuery);
