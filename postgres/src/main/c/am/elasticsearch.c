...
static void es_wait_for_index_availability(ZDBIndexDescriptor *indexDescriptor) {
    ...
    appendStringInfo(endpoint, "%s_cluster/health/%s?wait_for_status=yellow", indexDescriptor->url, indexDescriptor->fullyQualifiedName);
...
void elasticsearch_createNewIndex(ZDBIndexDescriptor *indexDescriptor, int shards, char *fieldProperties) {
    ...
    appendStringInfo(indexSettings, "{"
            ...
            "   }",
					 ...
					 lookup_analysis_thing(CurrentMemoryContext, "zdb_normalizers")
	);
...
char *elasticsearch_dumpQuery(ZDBIndexDescriptor *indexDescriptor, char *userQuery) {
    ...
    StringInfo endpoint           = makeStringInfo();
    StringInfo response;
    bool       useInvisibilityMap = strstr(userQuery, "#limit") != NULL || strstr(userQuery, "#options") != NULL || strstr(userQuery, "#expand") != NULL || indexDescriptor->options != NULL;
...
ZDBSearchResponse *elasticsearch_searchIndex(ZDBIndexDescriptor *indexDescriptor, char **queries, int nqueries, uint64 *nhits, bool wantScores, bool needSort) {
    ...
    ZDBScore          max_score;
    bool              useInvisibilityMap = strstr(queries[0], "#limit") != NULL || strstr(queries[0], "#options") != NULL || strstr(queries[0], "#expand") != NULL || indexDescriptor->options != NULL;
