...
static void checkForBulkError(StringInfo response, char *type) {
    ...
    if (errorsText == NULL)
        elog(IsTransactionState() ? ERROR
                                  : WARNING, "Unexpected response from elasticsearch during %s: %s", type, response->data);
    else {
...
static void checkForRefreshError(StringInfo response) {
    ...
    if (failedText == NULL)
        elog(IsTransactionState() ? ERROR
                                  : WARNING, "Unexpected response from elasticsearch during _refresh: %s", response->data);
    else {
...
void elasticsearch_refreshIndex(ZDBIndexDescriptor *indexDescriptor) {
    ...
    if (indexDescriptor->current_pool_index == -1) {
        i=0;
        many = indexDescriptor->shards;
    } else {
        i = indexDescriptor->current_pool_index;
        many = i+1;
    }
...
void elasticsearch_batchInsertFinish(ZDBIndexDescriptor *indexDescriptor) {
    ...
    if (batch) {
        ...
        if (batch->nrequests > 0)
            elog(LOG, "Indexed %d rows in %d requests for %s", batch->nprocessed,
