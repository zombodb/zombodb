...
SELECT assert(count(*), 310546, 'count(word:*) from words') FROM words WHERE zdb('words', ctid) ==> 'word:*';
...
SELECT assert(count(*), 165240, 'count(id:*) from so_posts') FROM so_posts WHERE zdb('so_posts', ctid) ==> 'id:*';
...
SELECT assert(count(*), 136990, 'count(id:*) from so_users') FROM so_users WHERE zdb('so_users', ctid) ==> 'id:*';
