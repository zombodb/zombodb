...
CREATE INDEX idxissue_66_test ON issue_66.test USING zombodb (zdb('issue_66.test', ctid), zdb_to_jsonb(test)) WITH (url='http://localhost:9200/');

CREATE SCHEMA issue_66_other;
CREATE TABLE issue_66_other.test(id serial8 not null primary key);
CREATE INDEX idxissue_66_test ON issue_66_other.test USING zombodb (zdb('issue_66_other.test', ctid), zdb_to_jsonb(test)) WITH (url='http://localhost:9200/');
