CREATE TABLE prox_highlight (id bigint not null primary key, body fulltext_with_shingles);
INSERT INTO prox_highlight (id, body) VALUES (1, ' CONFIDENTIAL Respect Integrity Communication Excellence
...
CREATE INDEX idxprox ON prox_highlight USING zombodb (zdb('prox_highlight', ctid), zdb(prox_highlight)) WITH (url='localhost:9200/');

SELECT * FROM zdb_highlight('prox_highlight'::REGCLASS, '( ( ( body:(Lay w/3 Enron w/3 Energy) ) ) )'::TEXT, 'id IN (''1'')'::TEXT, '{"body"}'::TEXT[])
ORDER BY "primaryKey", "fieldName", "arrayIndex", "position";

SELECT * FROM zdb_highlight('prox_highlight'::REGCLASS, '( ( ( body:(Lay w/4 Enron w/4 Energy) ) ) )'::TEXT, 'id IN (''1'')'::TEXT, '{"body"}'::TEXT[])
ORDER BY "primaryKey", "fieldName", "arrayIndex", "position";

DROP TABLE prox_highlight;