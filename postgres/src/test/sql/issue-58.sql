...
CREATE INDEX idxc ON c USING zombodb(zdb('c', ctid), zdb_to_jsonb(C)) WITH (url='http://localhost:9200/');
...
CREATE INDEX idxc_shadow ON c USING zombodb(issue_58_func('c', ctid), zdb_to_jsonb(C)) WITH (shadow='idxc');
