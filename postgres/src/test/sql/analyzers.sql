...
CREATE INDEX idxanalyzers_test ON analyzers_test USING zombodb (zdb(analyzers_test), zdb_to_json(analyzers_test)) WITH (url='http://localhost:9200/');
SELECT
  attname,
  (SELECT zdb(analyzers_test) ==> (attname || ':test') :: TEXT
