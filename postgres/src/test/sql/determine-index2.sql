...
CREATE INDEX es_sub ON mam_doc_sub USING zombodb (zdb('public.mam_doc_sub':: REGCLASS, ctid), zdb_to_jsonb(mam_doc_sub.*)) WITH (url='http://localhost:9200/', replicas=1, shards=5);
CREATE INDEX es_mam_doc ON mam_doc USING zombodb (zdb('public.mam_doc':: REGCLASS, ctid), zdb_to_jsonb(mam_doc.*)) WITH (url='http://localhost:9200/', replicas=1, shards=5);
...
COST 1;
CREATE INDEX es_idx_zdb_mam_doc_to_sub ON public.mam_doc USING zombodb (zdb_mam_doc_to_sub('public.mam_doc', ctid), zdb_to_jsonb(mam_doc.*)) WITH (shadow='es_mam_doc', OPTIONS ='mam_doc_sub_data:(pk_mam_doc=<mam_doc_sub.es_sub>fk_mam_doc)');
