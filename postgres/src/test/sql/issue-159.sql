...
CREATE INDEX idxissue159 ON issue159 USING zombodb (zdb(issue159), zdb_to_json(issue159)) WITH (url='http://localhost:9200/');
...
FROM issue159
WHERE zdb(issue159) ==> '( (#expand<groupid=<this.index>groupid>( ( pk_id = 239558) )) )'
ORDER BY pk_id;


SELECT pk_id, groupid, has_att
FROM issue159
WHERE zdb(issue159) ==> '( (#expand<groupid=<this.index>groupid>( ( pk_id = 239558) #filter(has_att = A) )) )'
ORDER BY pk_id;


SELECT pk_id, groupid, has_att
FROM issue159
WHERE zdb(issue159) ==> '( (#expand<groupid=<this.index>groupid>( ( pk_id = 239558) #filter(has_att = Y) )) )'
ORDER BY pk_id;
...
CREATE INDEX es_issue_159_data ON issue_159.data USING zombodb (public.zdb(data), public.zdb(data.*)) WITH (url='http://127.0.0.1:9200/', options='pk_data = <var.es_issue_159_var>pk_var,pk_data = <vol.es_issue_159_vol>pk_vol', shards='3', replicas='1');
CREATE INDEX es_issue_159_var ON issue_159.var USING zombodb (public.zdb(var), public.zdb(var.*)) WITH (url='http://127.0.0.1:9200/', shards='3', replicas='1');
CREATE INDEX es_issue_159_vol ON issue_159.vol USING zombodb (public.zdb(vol), public.zdb(vol.*)) WITH (url='http://127.0.0.1:9200/', shards='3', replicas='1');
