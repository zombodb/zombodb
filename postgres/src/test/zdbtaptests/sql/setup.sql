...
CREATE INDEX es_unit_tests_data
  ON unit_tests.data USING zombodb (zdb('unit_tests.data', ctid), zdb( DATA.*)) WITH (block_routing_field='pk_data', url=:zombodb_url, OPTIONS ='pk_data = <var.es_unit_tests_var>pk_var,pk_data = <vol.es_unit_tests_vol>pk_vol', shards=32, replicas='1');
--**********************************************************************************************************************
...
CREATE INDEX es_unit_tests_var
  ON unit_tests.var USING zombodb (zdb('unit_tests.var', ctid), zdb(var.*)) WITH (block_routing_field='pk_var', url=:zombodb_url, shards=32, replicas='1');
--**********************************************************************************************************************
...
CREATE INDEX es_unit_tests_vol
  ON unit_tests.vol USING zombodb (zdb('unit_tests.vol', ctid), zdb(vol.*)) WITH (block_routing_field='pk_vol', url=:zombodb_url, shards=32, replicas='1');
--**********************************************************************************************************************
...
CREATE INDEX es_unit_tests_data_same
  ON unit_tests.data_same USING zombodb (zdb('unit_tests.data_same', ctid), zdb(data_same.*)) WITH (url=:zombodb_url, OPTIONS ='id = <var_same.es_unit_tests_var_same>id, id = <vol_same.es_unit_tests_vol_same>id', shards=32, replicas='1');
CREATE INDEX es_unit_tests_var_same
  ON unit_tests.var_same USING zombodb (zdb('unit_tests.var_same', ctid), zdb(var_same.*)) WITH (url=:zombodb_url, shards=32, replicas='1');
CREATE INDEX es_unit_tests_vol_same
  ON unit_tests.vol_same USING zombodb (zdb('unit_tests.vol_same', ctid), zdb(vol_same.*)) WITH (url=:zombodb_url, shards=32, replicas='1');
...
CREATE INDEX es_unit_tests_case_name
  ON unit_tests.case_name USING zombodb (zdb('unit_tests.case_name':: REGCLASS, ctid), zdb(case_name.*)) WITH (url=:zombodb_url, shards=32, replicas='1');
